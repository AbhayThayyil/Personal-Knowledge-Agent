...
async def stream_chat_response(collection_id: int, question: str) -> AsyncIterator[str]:
    query_embedding = await embed_text(question)
    chunks = search(collection_id, query_embedding, top_k=settings.chat_top_k)

    if not chunks:
        yield _sse_event({"type": "token", "content": "I couldn't find any relevant documents to answer that."})
